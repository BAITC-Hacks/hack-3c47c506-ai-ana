...
Конфигурация прошла `docker compose config --quiet` и проверку подстановки нестандартного порта. Фактическая сборка образов и запуск требуют установленного и доступного Docker Engine; до выполнения этих проверок их нельзя считать подтверждёнными.
